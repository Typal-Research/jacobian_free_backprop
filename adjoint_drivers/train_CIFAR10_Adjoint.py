...
class CIFAR10_CNN(nn.Module):
    def __init__(self, lat_dim, device, s_hi=1.0, inf_dim=10):
        ...
        self.conv2 = nn.Conv2d(in_channels=128, out_channels=237, kernel_size=3, stride=1)
        self.conv3 = nn.Conv2d(in_channels=237, out_channels=256, kernel_size=3, stride=1)

        self.fc_input1    = nn.Linear(in_features=4096, out_features=lat_dim)
    ...
    def latent_space_forward(self, u, v):
        u = self.leakyrelu(self.fc_u1(u))
        u = self.leakyrelu(self.fc_u2(u))
        output = 0.9*u + v
        return output
...
lat_dim     = 45
s_hi        = 1.0
T           = CIFAR10_CNN(lat_dim, device, s_hi=s_hi).to(device)
num_classes = 10
eps = 1.0e-4
max_depth = 500
...
lr_scheduler  = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epochs, eta_min=1e-6, last_epoch=-1); print('USING COSINE ANNEALING')
checkpt_path  = './models/'
...
max_iter_cg   = 500
tol_cg        = eps
save_dir = 'SHIFT3e-2_Adjoint_tolcg1e-4_eps1e-4_saved_weights.pth'
