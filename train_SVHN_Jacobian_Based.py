...
from Networks import SVHN_FPN_Jacobian_Based, BasicBlock
from utils import svhn_loaders, compute_fixed_point
...
res_layers = 1
T       = SVHN_FPN_Jacobian_Based(res_layers=res_layers, num_channels=64, contraction_factor=contraction_factor, block=BasicBlock, num_blocks=num_blocks).to(device)
eps         = 1.0e-4
max_depth   = 200
...
print('\n\n  LATENT BATCH NORM, NO SHIFT, max_iter_cg = ', max_iter_cg, ', max_depth = ', max_depth)
...
for epoch in range(max_epochs): 
    ...
    with tqdm(total=tot, unit=" batch", leave=False, ascii=True) as tepoch:
        ...
        for idx, (d, labels) in enumerate(train_loader):         
            ...
            with torch.no_grad():
                ...
                u, depth = compute_fixed_point(T, Qd, max_depth, device, eps=eps)
            ...
            dRduT_dldu  = torch.autograd.grad(outputs=Ru, inputs = u, grad_outputs=dldu, retain_graph=True, create_graph = True, only_inputs=True)[0];
            rhs = dldu - dRduT_dldu 
            #-----------------------------------------------------------------------
            ...
            def JTJ_matvec(v, u=u, Ru=Ru):
                ...
                dRduT_mv = torch.autograd.grad(outputs = Ru, inputs = u, grad_outputs = v, retain_graph=True, create_graph = True, only_inputs=True)[0]
                JTv     = v - dRduT_mv

                # compute J * JTv
                # trick for computing J * (JTv): # take take derivative d(JTv)/dv * JTv = J * JTv
                dRdu_JTv    = torch.autograd.grad(outputs = JTv, inputs = v, grad_outputs = JTv.detach(), retain_graph=True, create_graph = True, only_inputs=True)[0]
            ...
            JTJinv_rhs, info = cg_batch(JTJ_matvec, rhs, M_bmm=None, X0=None, rtol=0, atol=tol_cg, maxiter=max_iter_cg, verbose=False)
            JTJinv_rhs = JTJinv_rhs.squeeze(2) # JTJinv_v has size (batch_size x n_hidden_features), n_rhs is squeezed
            ...
            if info['optimal'] == True:
                ...
                # computes dRdTheta * JTJinv_v = dRdTheta * Jinv * dSdu * dldS
    ...
    test_loss, test_acc, correct, depth_test = get_test_stats(T, test_loader, criterion, eps, max_depth)
