...
class MNIST_FPN(nn.Module):
    ...
    def name(self):
        '''
            ...
            This is useful for saving the model and choosing which forward prop to use. 
        '''
...
class SVHN_FPN(nn.Module):
    def __init__(self, res_layers=4, num_channels=32, contraction_factor=0.1,
                 momentum=0.1, block=BasicBlock, num_blocks=[1,1,1], architecture='FPN'):
        super().__init__()
...
class CIFAR10_FPN(nn.Module):
    ...
    def forward(self, d: image, eps=1.0e-3, max_depth=100, 
        ...
        if self.architecture == 'Explicit' or self.architecture == 'Explicit_Unaugmented':
            return forward_explicit(self, d, eps=eps, max_depth=max_depth, 
                        depth_warning=False)
        else:
            return forward_implicit(self, d, eps=eps, max_depth=max_depth,
