...
class MNIST_FPN_Jacobian_Based(nn.Module):
    def __init__(self, res_layers=4, num_channels=32, contraction_factor=0.1,
                 momentum=0.1):
        super().__init__()
...
class SVHN_FPN_Jacobian_Based(nn.Module):
    def __init__(self, res_layers=4, num_channels=32, contraction_factor=0.1,
                 momentum=0.1, block=BasicBlock, num_blocks=[1,1,1]):
        super().__init__()
        self.avg_pool = nn.AvgPool2d(kernel_size=2)
...
class CIFAR10_FPN(nn.Module):
    def __init__(self, res_layers=16, num_channels=35, contraction_factor=0.5,
        ...
        self.data_convs = nn.ModuleList([nn.Sequential(nn.Conv2d(in_channels=in_chan(i), 
    ...
    def forward(self, d: image, eps=1.0e-3, max_depth=100, 
        ...
        with torch.no_grad():
            self.depth = 0.0
            u = torch.zeros((d.size()[0], self._channels, 10, 10), 
                                    device=self.device())
            u_prev = np.Inf*torch.ones(u.shape, device=self.device())            
            Qd = self.data_space_forward(d)
            all_samp_conv = False
...
class CIFAR10_FPN_Jacobian_Based(nn.Module):
    def __init__(self, res_layers=16, num_channels=35, contraction_factor=0.5,
                 momentum=0.1, lat_layers=5):
        super().__init__() 
        self.avg_pool = nn.AvgPool2d(kernel_size=2)
...
class CIFAR10_FPN_Unaugmented_Jacobian_Based(nn.Module):
    def __init__(self, res_layers=4, num_channels=32, contraction_factor=0.1,
                 momentum=0.1, block=BasicBlock, num_blocks=[1,1,1]):
        super().__init__()
        self.avg_pool = nn.AvgPool2d(kernel_size=2)
    ...
    def name(self):
        return 'CIFAR10_FPN_Unaugmented_Jacobian_Based'
